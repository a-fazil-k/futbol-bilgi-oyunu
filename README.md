...
Giriş ekranında kullanıcı adını girdikten sonra iki moddan biri seçilir:

- **🌍 Ülke - Kulüp:** Bir oyuncu Kulüp, diğeri Ülke seçer. Amaç, hem o kulüpte
  oynamış hem o ülkeden olan futbolcuyu bulmak.
- **🛡️ Kulüp - Kulüp:** İki oyuncu da birer Kulüp seçer. Amaç, kariyerinde
  **iki kulüpte de forma giymiş** futbolcuyu bulmak. İki oyuncu aynı kulübü
  seçerse (bariz biçimde her futbolcunun eşleşeceği bir round olmasın diye),
  sunucu ikinci gelen seçimi reddedip "rakibin de bu kulübü seçti, farklı bir
  kulüp dene" uyarısı verir — süre bitene kadar farklı bir kulüp seçmek gerekir.
  Süre dolduğunda hâlâ boş/çakışan seçim varsa sunucu otomatik olarak birbirinden
  farklı iki kulüp atar.

Eşleştirme moda göre ayrı kuyruklarda yapılır: Ülke-Kulüp seçen biri yalnızca
yine Ülke-Kulüp seçen biriyle, Kulüp-Kulüp seçen biri de yalnızca yine
Kulüp-Kulüp seçen biriyle eşleşir.

## Oyun Akışı

1. Kullanıcı adı gir → mod seç (Ülke-Kulüp / Kulüp-Kulüp) → Lobi → otomatik eşleşme (2 oyuncu).
2. **Seçim aşaması (30 sn):** Moda göre kriterler seçilir (autocomplete).
3. **Tahmin aşaması (30 sn):** Kriterler açılır, her iki oyuncu da futbolcu adı
   yazar (autocomplete — isim, soyisim veya isim içinde geçen herhangi bir
   parça ile arama yapılabilir). İlk doğru cevabı veren +1 puan alır. Yanlış
   cevap 3 saniye cevap kilidi getirir.
4. Ülke-Kulüp modunda roller her tur değişir. Toplam **5 tur** sonunda en
   yüksek skor kazanır.
